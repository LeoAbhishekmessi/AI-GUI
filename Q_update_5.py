...
def save_question_answer(exam_id, user_id, question_id, question_type, answer, SESSION_TOKEN):
    ...
    try:
        ...
        # Prepare the payload - try both naming conventions
        payload = {
            "exam_id": exam_id,
            "user_id": user_id,
            "question_id": question_id,  # Original key
            "questionId": question_id,   # Alternative key
...
def format_answer_for_api(question_type, answer):
    ...
    if question_type == "1":  # Descriptive
        ...
        # Send array of selected indices
        # Convert to list of strings if needed
        return [str(idx) for idx in answer]
...
class ExamPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.timer = QTimer(self)
        self.remaining_seconds = 0  # Total remaining time in seconds
    ...
    def set_exam_details(self, exam_details):
        ...
        print(SESSION_TOKEN,"a7891dddtdtydtdvbcbc---------------------------------------")
    ...
    def auto_submit_exam(self):
        ...
        try:
            # We've already saved answers one by one, so just inform the user
            
            # Show success message
            success_box = QMessageBox()
            success_box.setWindowTitle("Exam Submitted")
            success_box.setText("Your exam has been submitted automatically as time expired.")
            success_box.exec()
    ...
    def store_user_answer(self):
        ...
        print(f"DEBUG: Current question keys: {current_question.keys()}")
        ...
        print(f"DEBUG: Using question ID: {question_id}")
    ...
    def submit_exam(self):
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            ...
            # Here you would send the answers to your server if any weren't saved yet
